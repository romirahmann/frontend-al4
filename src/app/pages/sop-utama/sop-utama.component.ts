...
export class SopUtamaComponent {
  ...
  constructor(private sopService: SopService, private http: HttpClient, private authService: AuthService, private el: ElementRef, private renderer: Renderer2 ){}
  ...
  getBreadCrumbItems(){
    this.breadCrumbItems = [
      { label: `Dashboard ${this.namaUserRole}` },
    ];
  }
  ...
  fetchPersentageUserLogin(){
    this.sopService.getPercentageProgressByArea(this.userId, this.areaId).subscribe(
      (res: any) => {
  ...
  getUserPercentage(groupId: number): number {
    const groupData = this.totalPercentage.find((data: { groupId: number }) => data.groupId === groupId);
    ...
    this.sopService.getDataPercentageByGroup(groupId).subscribe(
      (res: any) => {
        ...
        if (lengthData === 0) {
          ...
          this.totalPercentage.push({ groupId, percentage: Math.round(persentaseByGroup) });
        }        
      }, (error: any) => {
    ...
    this.sopService.getDataPercentageTeam(groupId, areaId).subscribe(
      (res: any) => {
        ...
        if (lengthDataUserByTeam.length === 0) { 
          ...
          this.percentageTeam.push(Math.round(totalPercentage / lengthDataUserByTeam));
         }
        }, (error: any) => {
